...
def build_model():
    ...
    ldsys3 = nn.layers.DenseLayer(ldsys2drop, num_units=2, b=nn.init.Constant(0.1), nonlinearity=nn.nonlinearities.identity)

    l_systole = MuSigmaErfLayer(ldsys3)
    ...
    lddia3 = nn.layers.DenseLayer(lddia2drop, num_units=2, b=nn.init.Constant(0.1), nonlinearity=nn.nonlinearities.identity)

    l_diastole = MuSigmaErfLayer(lddia3)
