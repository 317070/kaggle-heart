...
def reverse_softmax(x):
    return nn.nonlinearities.softmax(-x)
...
def build_model():
    ...
    ld3a = batch_norm(nn.layers.dnn.Conv2DDNNLayer(l0_frames_d3,  W=nn.init.Orthogonal("relu"), filter_size=(3,3), num_filters=16, stride=(1,1), pad="same", nonlinearity=nn.nonlinearities.rectify))
    ld3b = batch_norm(nn.layers.dnn.Conv2DDNNLayer(ld3a,  W=nn.init.Orthogonal("relu"), filter_size=(3,3), num_filters=16, stride=(1,1), pad="same", nonlinearity=nn.nonlinearities.rectify))
    ld3c = batch_norm(nn.layers.dnn.Conv2DDNNLayer(ld3b,  W=nn.init.Orthogonal("relu"), filter_size=(3,3), num_filters=16, stride=(1,1), pad="same", nonlinearity=nn.nonlinearities.rectify))
    ld3o = batch_norm(nn.layers.dnn.Conv2DDNNLayer(ld3c,  W=nn.init.Orthogonal("relu"), filter_size=(3,3), num_filters=16, stride=(1,1), pad="same", nonlinearity=nn.nonlinearities.rectify))

    ld2i = nn.layers.ConcatLayer([l0_frames_d2, upsample(ld3o)], axis=1)
    ld2a = batch_norm(nn.layers.dnn.Conv2DDNNLayer(ld2i,  W=nn.init.Orthogonal("relu"), filter_size=(5,5), num_filters=32, stride=(1,1), pad="same", nonlinearity=nn.nonlinearities.rectify))
    ld2b = batch_norm(nn.layers.dnn.Conv2DDNNLayer(ld2a,  W=nn.init.Orthogonal("relu"), filter_size=(5,5), num_filters=32, stride=(1,1), pad="same", nonlinearity=nn.nonlinearities.rectify))
    ld2c = batch_norm(nn.layers.dnn.Conv2DDNNLayer(ld2b,  W=nn.init.Orthogonal("relu"), filter_size=(5,5), num_filters=32, stride=(1,1), pad="same", nonlinearity=nn.nonlinearities.rectify))
    ld2d = batch_norm(nn.layers.dnn.Conv2DDNNLayer(ld2c,  W=nn.init.Orthogonal("relu"), filter_size=(5,5), num_filters=32, stride=(1,1), pad="same", nonlinearity=nn.nonlinearities.rectify))
    ld2o = batch_norm(nn.layers.dnn.Conv2DDNNLayer(ld2d,  W=nn.init.Orthogonal("relu"), filter_size=(5,5), num_filters=32, stride=(1,1), pad="same", nonlinearity=nn.nonlinearities.rectify))

    ld1i = nn.layers.ConcatLayer([l0_frames_d1, upsample(ld2o)], axis=1)
    ld1a = batch_norm(nn.layers.dnn.Conv2DDNNLayer(ld1i,  W=nn.init.Orthogonal("relu"), filter_size=(5,5), num_filters=32, stride=(1,1), pad="same", nonlinearity=nn.nonlinearities.rectify))
    ld1b = batch_norm(nn.layers.dnn.Conv2DDNNLayer(ld1a,  W=nn.init.Orthogonal("relu"), filter_size=(5,5), num_filters=32, stride=(1,1), pad="same", nonlinearity=nn.nonlinearities.rectify))
    ld1c = batch_norm(nn.layers.dnn.Conv2DDNNLayer(ld1b,  W=nn.init.Orthogonal("relu"), filter_size=(5,5), num_filters=32, stride=(1,1), pad="same", nonlinearity=nn.nonlinearities.rectify))
    ld1d = batch_norm(nn.layers.dnn.Conv2DDNNLayer(ld1c,  W=nn.init.Orthogonal("relu"), filter_size=(5,5), num_filters=32, stride=(1,1), pad="same", nonlinearity=nn.nonlinearities.rectify))
    ld1o = batch_norm(nn.layers.dnn.Conv2DDNNLayer(ld1d,  W=nn.init.Orthogonal("relu"), filter_size=(5,5), num_filters=32, stride=(1,1), pad="same", nonlinearity=nn.nonlinearities.rectify))

    ld0i = nn.layers.ConcatLayer([l0_frames_d0, upsample(ld1o)], axis=1)
    ld0a = batch_norm(nn.layers.dnn.Conv2DDNNLayer(ld0i,  W=nn.init.Orthogonal("relu"), filter_size=(5,5), num_filters=32, stride=(1,1), pad="same", nonlinearity=nn.nonlinearities.rectify))
    ld0b = batch_norm(nn.layers.dnn.Conv2DDNNLayer(ld0a,  W=nn.init.Orthogonal("relu"), filter_size=(5,5), num_filters=32, stride=(1,1), pad="same", nonlinearity=nn.nonlinearities.rectify))
    ld0c = batch_norm(nn.layers.dnn.Conv2DDNNLayer(ld0b,  W=nn.init.Orthogonal("relu"), filter_size=(5,5), num_filters=32, stride=(1,1), pad="same", nonlinearity=nn.nonlinearities.rectify))
    ld0d = batch_norm(nn.layers.dnn.Conv2DDNNLayer(ld0c,  W=nn.init.Orthogonal("relu"), filter_size=(5,5), num_filters=32, stride=(1,1), pad="same", nonlinearity=nn.nonlinearities.rectify))
    ld0o = batch_norm(nn.layers.dnn.Conv2DDNNLayer(ld0d,  W=nn.init.Orthogonal("relu"), filter_size=(5,5), num_filters=1, stride=(1,1), pad="same", nonlinearity=nn.nonlinearities.sigmoid))
    ...
    # Go back to a per slice model with diastole and systole attention
    l_slices_musigma_cm = nn.layers.ReshapeLayer(l_frames_musigma_cm, (batch_size * nr_slices, nr_frames_subsampled, 2))  # (bxs, t, 2)
    
    attention_frames = nn.layers.ReshapeLayer(l0_frames_d2, (batch_size * nr_slices, nr_frames_subsampled, [2], [3]))
    l_att_1a = batch_norm(nn.layers.dnn.Conv2DDNNLayer(attention_frames,  W=nn.init.Orthogonal("relu"), filter_size=(5,5), num_filters=32, stride=(1,1), pad="same", nonlinearity=nn.nonlinearities.rectify))
    l_att_1b = batch_norm(nn.layers.dnn.Conv2DDNNLayer(l_att_1a,  W=nn.init.Orthogonal("relu"), filter_size=(5,5), num_filters=32, stride=(1,1), pad="same", nonlinearity=nn.nonlinearities.rectify))
    l_att_1 = nn.layers.dnn.MaxPool2DDNNLayer(l_att_1b, pool_size=(2,2), stride=(2,2))

    l_att_2a = nn.layers.DenseLayer(l_att_1, num_units=128, W=nn.init.Orthogonal("relu"), b=nn.init.Constant(0.1), nonlinearity=nn.nonlinearities.rectify)
    l_att_2b = nn.layers.DenseLayer(l_att_2a, num_units=128, W=nn.init.Orthogonal("relu"), b=nn.init.Constant(0.1), nonlinearity=nn.nonlinearities.rectify)
    l_att_2 = nn.layers.DenseLayer(l_att_2a, num_units=nr_frames_subsampled, W=nn.init.Orthogonal("relu"), b=nn.init.Constant(0.1), nonlinearity=None)
    l_att_sys = nn.layers.NonlinearityLayer(l_att_2, nonlinearity=nn.nonlinearities.softmax)
    l_att_dia = nn.layers.NonlinearityLayer(l_att_2, nonlinearity=reverse_softmax)

    l_slices_musigma_cm_sys = layers.SelectWithAttentionLayer([l_slices_musigma_cm, l_att_sys])  # (bxs, 2)
    l_slices_musigma_cm_dia = layers.SelectWithAttentionLayer([l_slices_musigma_cm, l_att_dia])  # (bxs, 2)
