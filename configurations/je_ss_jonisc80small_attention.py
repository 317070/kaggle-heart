...
def build_model(input_layer = None):
    ...
    # Subsample frames
    subsample_factor = 2
    l0_slices_subsampled = nn.layers.SliceLayer(l0, axis=1, indices=slice(0, NR_FRAMES, subsample_factor))
    ...
    l_att_d1 = nn.layers.DenseLayer(l5, num_units=32, W=nn.init.Orthogonal("relu"), b=nn.init.Constant(0.1), nonlinearity=nn.nonlinearities.rectify)
    l_att_d1re = nn.layers.ReshapeLayer(l_att_d1, (batch_size, nr_frames_subsampled * 32))
    l_att_d2 = nn.layers.DenseLayer(l_att_d1re, num_units=64, W=nn.init.Orthogonal("relu"), b=nn.init.Constant(0.1), nonlinearity=nn.nonlinearities.rectify)
    l_att = nn.layers.DenseLayer(l_att_d2, num_units=nr_frames_subsampled, W=nn.init.Orthogonal("relu"), b=nn.init.Constant(0.1), nonlinearity=None)
    l_att_sys = nn.layers.NonlinearityLayer(l_att, nonlinearity=nn.nonlinearities.softmax)
    l_att_dia = nn.layers.NonlinearityLayer(l_att, nonlinearity=reverse_softmax)


    # Reshape back to slicemodel
    ld3musigma_slices = nn.layers.ReshapeLayer(ld3musigma, (-1, nr_frames_subsampled, 2))

    l_slices_musigma_cm_sys = layers.SelectWithAttentionLayer([ld3musigma_slices, l_att_sys])  # (bxs, 2)
    l_slices_musigma_cm_dia = layers.SelectWithAttentionLayer([ld3musigma_slices, l_att_dia])  # (bxs, 2)
