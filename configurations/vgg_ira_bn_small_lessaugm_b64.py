...
learning_rate_schedule = {
    0: 0.001,
    int(max_nchunks * 0.25): 0.0003,
    int(max_nchunks * 0.5): 0.0001,
    int(max_nchunks * 0.75): 0.00003,
    int(max_nchunks * 0.85): 0.00001,
    int(max_nchunks * 0.95): 0.000003,
}
...
def build_model():
    ...
    l_d01 = nn.layers.DenseLayer(l, num_units=256, W=nn.init.Orthogonal(),
                                 b=nn.init.Constant(0.1))
    # l_d01 = batch_norm(l_d01)
    l_d02 = nn.layers.DenseLayer(nn.layers.dropout(l_d01, p=0.5), num_units=256, W=nn.init.Orthogonal(),
                                 b=nn.init.Constant(0.1))
    # l_d02 = batch_norm(l_d02)
    l_mu0 = nn.layers.DenseLayer(nn.layers.dropout(l_d02, p=0.5), num_units=1, nonlinearity=nn.nonlinearities.identity)
    # ---------------------------------------------------------------

    l_d11 = nn.layers.DenseLayer(l, num_units=256, W=nn.init.Orthogonal(),
                                 b=nn.init.Constant(0.1))
    # l_d11 = batch_norm(l_d11)
    l_d12 = nn.layers.DenseLayer(nn.layers.dropout(l_d11, p=0.5), num_units=256, W=nn.init.Orthogonal(),
                                 b=nn.init.Constant(0.1))
    # l_d12 = batch_norm(l_d12)
...
def build_updates(train_loss, model, learning_rate):
    updates = nn.updates.adam(train_loss, nn.layers.get_all_params(model.l_top, trainable=True), learning_rate)
    return updates
