...
train_data_iterator = data_iterators.SliceMetadata2DataGenerator(data_path='/data/dsb15_pkl/pkl_splitted/train',
                                                                 batch_size=chunk_size,
                                                                 transform_params=train_transformation_params,
                                                                 labels_path='/data/dsb15_pkl/train.csv',
                                                                 full_batch=True, random=True, infinite=True)

valid_data_iterator = data_iterators.SliceMetadata2DataGenerator(data_path='/data/dsb15_pkl/pkl_splitted/valid',
                                                                 batch_size=chunk_size,
                                                                 transform_params=valid_transformation_params,
                                                                 labels_path='/data/dsb15_pkl/train.csv',
                                                                 full_batch=False, random=False, infinite=False)

test_data_iterator = data_iterators.SliceMetadata2DataGenerator(data_path='/data/dsb15_pkl/pkl_validate',
...
def build_model():
    ...
    l_in_mtd = nn.layers.InputLayer((None, 3))
    l_ins = [l_in, l_in_spxl, l_in_mtd]
    ...
    l_last_conv = max_pool(l)
    l = nn.layers.ConcatLayer([nn.layers.flatten(l_last_conv), l_in_mtd])  # add metadata
    ...
    return namedtuple('Model', ['l_ins', 'l_outs', 'l_targets', 'l_top', 'regularizable_layers'])(l_ins, l_outs,
                                                                                                  l_targets, l_top,
                                                                                                  [l_d01, l_d02, l_d11,
                                                                                                   l_d12])
