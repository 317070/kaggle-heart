        ...
        while True:
            ...
            for pos in xrange(0, len(rand_idxs), self.batch_size):
                ...
                for i, j in enumerate(idxs_batch):
                    ...
                    if self.data_prep_fun == 'transform_norm_rescale_after':
                        x_batch[i], targets_zoom = data.transform_norm_rescale_after(slice_data, metadata,
            ...
            for pos in xrange(0, len(rand_idxs), self.batch_size):
                ...
                for i, idx in enumerate(idxs_batch):
                    ...
                    for j, sp in enumerate(slice_paths):
                        ...
                        if self.data_prep_fun == 'transform_norm_rescale_after':
                            x_batch[i, j], targets_zoom = data.transform_norm_rescale_after(slice_data,
                                                                                            slicepath2metadata[sp],
                                                                                            self.transformation_params,
                                                                                            roi=slice_roi,
                                                                                            random_augmentation_params=random_params)
                        else:
                            x_batch[i, j], targets_zoom = data.transform_norm_rescale(slice_data,
                                                                                      slicepath2metadata[sp],
                                                                                      self.transformation_params,
                                                                                      roi=slice_roi,
                                                                                      random_augmentation_params=random_params)
