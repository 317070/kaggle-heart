...
class IraLayer(nn.layers.MergeLayer):
    def __init__(self, incomings, trainable_scale=False, **kwargs):
        super(IraLayer, self).__init__(incomings, **kwargs)

        if trainable_scale:
            self.W_mu = self.add_param(nn.init.Constant(-2.3), (1,), name="W")
            self.b_mu = self.add_param(nn.init.Constant(-20.), (1,), name="b")

            self.W_sigma = self.add_param(nn.init.Constant(-2.3), (1,), name="W")
            self.b_sigma = self.add_param(nn.init.Constant(-20.), (1,), name="b")
    ...
    def get_output_for(self, inputs, **kwargs):
        mu_a, sigma_a, mu_b, sigma_b, slice_mask, slicelocs = inputs
        ...
        # Compute mu and sigma per patient
        mu_volume_patient = np.pi / 4. * T.sum(mu_volumes, axis=1, keepdims=True)
        sigma_volume_patient = np.pi / 4. * T.sqrt(T.sum(var_volumes, axis=1, keepdims=True))
