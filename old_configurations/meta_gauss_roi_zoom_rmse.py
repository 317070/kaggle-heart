...
def build_model():
    ...
    return namedtuple('Model', ['l_ins', 'l_outs', 'l_targets', 'l_top', 'train_params',
                                'submodel', 'mu_layers', 'sigma_layers'])(
        l_ins, l_outs,
        l_targets,
        l_top,
        train_params,
        submodel,
        mu_layers, sigma_layers)


def build_objective(model, deterministic=False):
    p0 = nn.layers.get_output(model.l_outs[0], deterministic=deterministic)
    m0 = nn.layers.get_output(model.mu_layers[0], deterministic=deterministic)
    sigma0 = nn.layers.get_output(model.sigma_layers[0], deterministic=deterministic)
    ...
    m1 = nn.layers.get_output(model.mu_layers[1], deterministic=deterministic)
    sigma1 = nn.layers.get_output(model.sigma_layers[1], deterministic=deterministic)
    ...
    rmse = 0.5 * T.sqrt(T.mean((m0 - t0) ** 2)) + T.sqrt(T.mean((m1 - t1) ** 2))

    l2_sigma = 0.5 * (T.mean(sigma0 ** 2) + T.mean(sigma1 ** 2))

    return crps + 0.001* rmse + 0.008 * l2_sigma
