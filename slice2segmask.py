...
def get_slice2roi(data_path, plot=False):
    ...
    for p in patient_paths:
        ...
        #then, if it has been interrupted:
        #if(pid > x)
     ...
     #   slice2roi[pid] = {}
        ...
        for slice_group in grouped_sax_slices:
            ...
            numslices = len(slice_group)
            centerslice=numslices/2
            testblock = slice_group[centerslice]['data'].copy()#             
            minradius = 15
            maxradius = 65
            x_axis= roi_center[0]
            ...
            x_radius= roi_radii[0]
            y_radius= roi_radii[1]

          
            #TODO: output format?
            #TODO: better form
            ROImask = np.zeros_like(testblock[0])
            ...
            cc=cc[dum]
            ROImask[rr,cc]=1.
            g, bloodlabel, lowerb, upperb = extract_roi.extract_segmentation_model(testblock)
            slice_masks=[]
            rs_minradius = max(int(minradius / np.max(slice_group[0]['metadata']['PixelSpacing'])),1)
            rs_maxradius = max(int(maxradius / np.max(slice_group[0]['metadata']['PixelSpacing'])),1)
            ...
            for slice in slice_group:
               ...
               # slice_masks.append(masks)
                sid = slice['slice_id']
                slice2roi[sid] = {'roi_center': roi_center, 'roi_radii': roi_radii,'segmask':mask.astype(bool)}
...
#            for s in slice_group:
 #               sid = s['slice_id']
     #           slice2roi[pid][sid] = {'roi_center': roi_center, 'roi_radii': roi_radii,'masks':masks}
  #              slice2roi[pid][sid] = {'roi_center': roi_center, 'roi_radii': roi_radii}
        ...
        filename = data_path.split('/')[-1] +'_segmasks/'+ 'segmasks_patient_'+str(pid) +'.pkl'
